<template>
  <div class="flex-1 overflow-auto px-7 py-6">
    ...
    <div class="mb-7">
      ...
      <p class="text-zinc-500 text-[11px]">One key per app version. Keys are shown only at creation.</p>
    </div>

    <!-- Create form -->
    <div class="mb-8 bg-zinc-900/50 border border-zinc-800/60 rounded-lg p-5">
      <div class="text-[10px] text-zinc-500 uppercase tracking-widest mb-3">New key</div>
      ...
      <div v-if="newKey" class="mt-4 space-y-3">
        <div class="text-[10px] text-orange-500 uppercase tracking-widest mb-1.5">
          Key generated - copy it now, it won't be shown again
        </div>

        <!-- Raw key -->
        <div>
          <div class="text-[10px] text-zinc-500 mb-1">Hexadecimal</div>
          <div class="flex items-center gap-2">
            <code class="flex-1 bg-zinc-950 border border-zinc-800 rounded px-3 py-2 text-[11px] text-slate-400"
                  style="word-break: break-all">{{ newKey }}</code>
            <button @click="copyKey(newKey)"
    ...
    <div class="border border-zinc-800/60 rounded-lg overflow-hidden">
      ...
      <div v-if="loading" class="px-4 py-6 text-xs text-zinc-500">loading...</div>
            ...
            <td class="px-8 py-3">
              <span class="text-[10px] px-1.5 py-0.5 rounded border"
                    :class="v.active
                      ...
                      : 'text-zinc-500 bg-zinc-800/30 border-zinc-700/30'">
...
async function copyKey(text) {
  const which = text === newKey.value ? 'raw' : 'mc'
